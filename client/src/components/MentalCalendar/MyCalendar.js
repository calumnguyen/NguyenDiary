  ...
  const showConfirmationForDateChange = (changedTempDate) => {
    ...
    if (firstDate !== secondDate && props.formStarted) {
      confirmAlert({
        ...
        message:
          'Bạn đang điền ghi chú giữa chừng. Nếu bạn không hoàn tất, mọi thông tin sẽ bị mất và bạn có thể phải điền lại ghi chú từ đầu.',
        buttons: [
          {
            label: 'Xác nhận rời trang',
            onClick: () => {
              onDateChangeFinal(changedTempDate);
            },
          },
          {
            label: 'Không, quay lại điền ghi chú',
            onClick: () => {},
          },
        ],
      });
          ...
          <div className='col-sm-10 col-xs-10'>
            <p className='calendar_title'>
              {props.selectedUserId !== props.authUserId
                ? props.selectedUser.information.fullName
                : 'Cảm Xúc Của Mình'}
            </p>
          </div>
