...
export const confirmPassword = (userObj) => async (dispatch) => {
  dispatch({
    type: USER_LOADING,
  })
  const config = {
   ...
   try {
    ...
    const res = await axios.post(`/api/users/confirmpassword/${userObj.id}`, userObj, config);
    dispatch({
...
export const getAssignedArticles = (userId) => async (dispatch) => {
  dispatch({
    type: USER_LOADING,
  })
  const config = {
   ...
   try {
    const res = await axios.get(`/api/users/articles/${userId}`, config);
    dispatch({
      type: GET_ASSIGNED_ARTICLES,
      payload: res.data,
    })
  } catch (err) {
    const errors = err.response.data.errors;
